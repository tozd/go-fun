...
func TestTextTools(t *testing.T) { //nolint:paralleltest,tparallel
	...
	tests := []textTestCase{
		{
			"just_prompt",
			"Repeat the input twice, by concatenating the input string without any space. Do not explain anything. Return only the resulting string without any description.",
			nil,
			[]fun.InputOutput[string, string]{
